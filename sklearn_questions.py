...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        for x in enumerate(distances):
            neighbors_idx = np.argsort(x[1])[:self.n_neighbors]
            z = np.unique(self.y_[neighbors_idx], return_counts=True)
            y.append(z[0][np.argmax(z[1])])
        return np.array(y)
...
class MonthlySplit(BaseCrossValidator):
    ...
    def get_n_splits(self, X, y=None, groups=None):
        ...
        if time.dtype != 'datetime64[ns]':
            raise ValueError("time column wrong type: datetime64 expected")

        n_split = Xr[self.time_col].dt.strftime("%m/%y").nunique() - 1
        return n_split
