...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        for i in range(num_X):
            ...
            y_labels = self.y_train_[indexes]

            resultat.append(max(list(y_labels), key=list(y_labels).count))
...
class MonthlySplit(BaseCrossValidator):
    ...
    def get_n_splits(self, X, y=None, groups=None):
        ...
        if not pd.api.types.is_datetime64_any_dtype(X.index):
            raise ValueError('Careful the input is not in datetime format')

        n_splits = 0
        zip_date = zip(X.index.month, X.index.year)
        possibilities = set({(month, year) for (month, year) in zip_date})
    ...
    def split(self, X, y, groups=None):
        ...
        splits = np.array([[a1, a2, a3, a4] for [(a1, a2),
                                                 (a3, a4)] in splits])
        splits = splits[np.lexsort((splits[:, 1], splits[:, 0]))]

        for split in splits:
            m1, y1 = split[0], split[1]
            m2, y2 = split[2], split[3]
            mask1 = (X.index.month == m1) & (X.index.year == y1)
            mask2 = (X.index.month == m2) & (X.index.year == y2)
