...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        X_new = check_array(X)
        y_pred = np.zeros(X_new.shape[0], dtype=self.y_train_.dtype)
        for itr, pt in enumerate(X_new):
            distance = [
            ...
            c = Counter(y_values)
            y_pred[itr] = c.most_common(1)[0][0]
