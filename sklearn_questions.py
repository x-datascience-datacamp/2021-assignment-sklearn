...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        nearest_point = pairwise_distances_argmin(X, self.X_)

        return self.y_[nearest_point]
    ...
    def score(self, X, y):
        ...
        """
        score = np.sum(self.predict(X) == y)/y.size
...
class MonthlySplit(BaseCrossValidator):
    ...
    def get_n_splits(self, X, y=None, groups=None):
        ...
        if self.time_col != 'index':
            date = X[self.time_col].dt

        return date.to_period('M').nunique() - 1
