...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        y : ndarray, shape (n_test_samples,)
            Class labels_ for each test data sample.
        """
        X = check_array(X)
        ...
        for i, x_i in enumerate(distance_matrix):

            idx = x_i.argsort()[:self.n_neighbors]
            y_neighbors = self.y_[idx]
            y_pred[i] = max(list(y_neighbors), key=list(y_neighbors).count)
